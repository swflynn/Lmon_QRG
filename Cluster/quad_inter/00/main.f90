...
module QRG_Lmon_Spectra
  ...
  double precision,allocatable::sqrt_mass(:),mass(:),x0(:),U(:,:),r(:,:),alpha(:)
  !==============================================================================!
contains
  !==============================================================================!
  ...
  subroutine normal_to_cartesian(r,x,flag)
    !==============================================================================!
    !r(1:d2)    ==>x(1:d)     flag=.true.
    !x(1:d)       ==>r(1:d2)  flag=.false.
    !==============================================================================!
    !x0(d)        ==>initial cluster configuration
    !r(d2)      ==>coordinate we want to evaluate the potential at
  ...
  subroutine gradient_transform(force,force1)
    ...
    force1(1:d2)=0
    do i=1,d2
       force1(i)=force1(i)+sum(U(1:d1,i)*rr(1:d1))      !mass-scaled coordinates-->normal modes
    enddo
  ...
  subroutine matrix_elements_grad(i,j,S_ij,H_ij,GH_order,z,w)
    ...
    double precision,parameter :: ss=1d-6   ! need to check
    double precision :: S_ij,H_ij,r2,a_ij,E,V_ij,Lap,ss1
    double precision :: r_ij(d2),rr(d2),w(GH_order),z(GH_order),r1(d2),x(d),force(d),force1(d2),force0(d2)
...
!    if(i.ne.j) then  ! use quadratic interpolation of V
    if(GH_order<0) then  ! use quadratic interpolation of V
       ...
       call gradient_transform(force,force0) !convert the force into the normal coordinate frame
       ...
       do k=1,d2
          ...
          call gradient_transform(force,force1)  !convert the force into the normal coordinate frame
...
program main_spectra
  ...
  double precision,allocatable,dimension(:) :: omega,eigenvalues,z,w,force
